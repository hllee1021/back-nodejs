...
router.get('/question/like/:id',async(req,res)=>{
    ...
    async.waterfall([
        function(callback){
            if(user.like.includes(id)==false){
                ...
                User.findOneAndUpdate({_id:'601d1a703118730630b8d2de'},{like:id}).exec();
            }
            else{
                Question.findOneAndUpdate({_id:id},{$inc:{like:-1}}).exec();
                User.findOneAndUpdate({_id:'601d1a703118730630b8d2de'},{$pull:{like:id}}).exec();
                user.dislike.splice(user.dislike.indexOf(id),1);
            }
            callback(null);
        }],
        function(){
